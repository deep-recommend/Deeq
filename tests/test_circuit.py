...
from queen import Circuit, QueenGlobalSetting
from queen.backends.onequbitgate_decomposer import u_decomposer
from queen.utils import ignore_global_phase
...
def test_cx3(backend):
    '''Refer issues #76 (https://github.com/Queen/Queen/issues/76)'''
...
def test_switch_backend1():
    ...
    QueenGlobalSetting.set_default_backend("qasm_output")
    ...
    from queen.backends.qasm_output_backend import QasmOutputBackend
    assert c.run(output_prologue=False) == c.run(False,
                                                 backend=QasmOutputBackend())

    QueenGlobalSetting.set_default_backend("numpy")
...
def test_macro():
    ...
    QueenGlobalSetting.register_macro('foo', macro)
    try:
        assert np.allclose(Circuit().foo(1).run(), Circuit().h[1].run())
    finally:
        QueenGlobalSetting.unregister_macro('foo')
