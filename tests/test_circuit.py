...
from deeq import Circuit, DeeqGlobalSetting
from deeq.backends.onequbitgate_decomposer import u_decomposer
from deeq.utils import ignore_global_phase
...
def test_cx3(backend):
    '''Refer issues #76 (https://github.com/Deeq/Deeq/issues/76)'''
...
def test_switch_backend1():
    ...
    DeeqGlobalSetting.set_default_backend("qasm_output")
    ...
    from deeq.backends.qasm_output_backend import QasmOutputBackend
    assert c.run(output_prologue=False) == c.run(False,
                                                 backend=QasmOutputBackend())

    DeeqGlobalSetting.set_default_backend("numpy")
...
def test_macro():
    ...
    DeeqGlobalSetting.register_macro('foo', macro)
    try:
        assert np.allclose(Circuit().foo(1).run(), Circuit().h[1].run())
    finally:
        DeeqGlobalSetting.unregister_macro('foo')
